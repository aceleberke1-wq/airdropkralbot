...
$taskRunnerPath = Join-Path $taskRunnerDir "run_v5_kpi_bundle.cmd"
$runnerBody = @"
@echo off
setlocal
cd /d "$repoRoot"
...
if ($LASTEXITCODE -ne 0) {
  if ($RunLevel -eq "HIGHEST") {
    ...
    Write-Host "[info] To remove: powershell -ExecutionPolicy Bypass -File scripts/register_v5_kpi_tasks.ps1 -TaskName `"$TaskName`" -UnregisterOnly"
    exit 0
  }
  throw "Task registration failed: $TaskName"
}
